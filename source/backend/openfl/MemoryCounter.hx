package backend.openfl;

import utils.NovaUtil;
import openfl.display.Sprite;
import openfl.text.TextField;
import openfl.text.TextFormat;

class MemoryCounter extends Sprite {
	public var memoryText:TextField;
	public var memoryPeakText:TextField;

	public var memory:Float = 0;
	public var memoryPeak:Float = 0;

	public function new() {
		super();

		memoryText = new TextField();
		memoryPeakText = new TextField();

		for(label in [memoryText, memoryPeakText]) {
			label.autoSize = LEFT;
			label.x = 0;
			label.y = 0;
			label.text = "MEM";
			label.multiline = label.wordWrap = false;
			label.defaultTextFormat = new TextFormat('${Sys.getEnv("windir")}\\Fonts\\consola.ttf', 30, -1);
			label.selectable = false;
			addChild(label);
		}
		memoryPeakText.alpha = 0.5;
	}

	public function reload() {}

    public function currentMemUsage() {
		return cpp.vm.Gc.memInfo64(cpp.vm.Gc.MEM_INFO_USAGE);
    }

	public override function __enterFrame(t:Int) {
		if (alpha <= 0.05) return;
		super.__enterFrame(t);

		memory = currentMemUsage();
		if (memoryPeak < memory) memoryPeak = memory;
		memoryText.text = "Memory Usage: " + NovaUtil.getSizeString(memory);
		memoryPeakText.text = ' / ${NovaUtil.getSizeString(memoryPeak)}';

		memoryPeakText.x = memoryText.x + memoryText.width;
	}
}