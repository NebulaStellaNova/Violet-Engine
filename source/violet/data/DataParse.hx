...
class DataParse {
	...
	/**
	 * `@:jcustomparse(violet.data.DataParse.semverVersion)`
	 * @param json Contains the `pos` and `value` of the property.
	 * @param name The name of the property.
	...
	/**
	 * `@:jcustomparse(violet.data.DataParse.semverVersionRule)`
	 * @param json Contains the `pos` and `value` of the property.
	 * @param name The name of the property.
	...
	public static function eitherFloatOrFloats(json:Json, name:String):Null<Either<Float, Array<Float>>> {
		switch (json.value) {
			case JNumber(f):
				return Left(Std.parseFloat(f));
			case JArray(fields):
				return Right(fields.map((field) -> cast Tools.getValue(field)));
			default:
