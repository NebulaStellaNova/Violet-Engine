...
/**
 * CREDITS TO Funkin' Crew FOR THIS FILE.
 */
...
class DropShadowShader extends FlxShader {
	...
	/**
	 * The NovaSprite that the shader should get the frame data from.
	 * Needed to keep the drop shadow shader in the correct bounds and rotation.
	 */
	public var attachedSprite(default, set):NovaSprite;
	...
	function set_attachedSprite(spr:NovaSprite):NovaSprite {
		attachedSprite = spr;
	...
	public function loadAltMask(path:String):Void {
		altMaskImage = cast BetterBitmapData.fromFile(path);
	}
