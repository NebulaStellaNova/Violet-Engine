...
class Conductor {
	...
	inline public static function playMusic(path:String, forced:Bool = false):Void {
		path = Paths.file("music/" + path);
		var splitPath = path.split("/");
		var fileName = splitPath[splitPath.length-1];
		if (curMusic == fileName && !forced) return;
		curMusic = fileName;
		// trace(path + ".ogg");
		// trace(Paths.fileExists('$path/$path.ogg', true) + ' ' + Paths.folderExists(path));
		if (Paths.fileExists(path + ".ogg", true)) {
			loadMusic('$path.ogg');
			play();
		} else if (Paths.folderExists(path, true)) {
	...
	public static function loadMusic(music:String, ?afterLoad:FlxSound->Void):Void {
		...
		NovaUtils.playMusic(music.split('/').pop());

		// TODO: Implement TS:
		/* if (Paths.fileExists(music, true)) {
			audio.loadEmbedded(music);
			FlxG.sound.loadHelper(audio, 1, soundGroup);
			audio.persist = true;
		} else {
			trace('error:Could not find sound asset with path "$music"');
		} */

		if (Paths.fileExists(music.replace(".ogg", ".json"), true)) {
			try {
				data = ParseUtil.json(music.replace(".ogg", ""));
			} catch (e:Dynamic) {
				// do nothing
			}
		}
	...
	public static function loadSong(song:String, variant:String = '', ?afterLoad:FlxSound->Void):Void {
		...
		if (Paths.fileExists('songs/$song/song/${variant != '' ? '$variant/' : ''}Inst.ogg')) {
			audio.loadEmbedded('songs/$song/song/${variant != '' ? '$variant/' : ''}Inst.ogg');
			FlxG.sound.loadHelper(audio, 1, soundGroup);
			audio.persist = true;
		} else {
			trace('error:Could not find sound asset with path "${'songs/$song/song/${variant != '' ? '$variant/' : ''}Inst.ogg'}"');
		}
	...
	public static function addVocalTrack(song:String, suffix:String = '', variant:String = '', ?afterLoad:FlxSound->Void):FlxSound {
		...
		if (Paths.fileExists(Paths.vocal(song, suffix, variant))) {
			...
			trace('error:Could not find vocal track for song "$song" with suffix "$suffix"');
		}
	...
	inline public static function stepHit(curStep:Int):Void {
		// cast (FlxG.state, StateBackend)?.stepHit(curStep);
	}
	/**
