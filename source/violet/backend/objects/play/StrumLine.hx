...
class StrumLine extends FlxGroup {
	...
	public function new(chartData:_ChartStrumLine) {
		...
		if (chartData.vocalsSuffix == null || chartData.vocalsSuffix == "") vocals = Conductor.addAdditionalTrack(new FlxSound());
			...
			var vocalPath = "";
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix, PlayState.variation);
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix, PlayState.variation, false);
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix.substr(0), PlayState.variation);
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix.substr(0), PlayState.variation, false);
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix, '');
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix, '', false);
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix.substr(0), '');
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix.substr(0), '', false);
			if (vocalPath == "") vocalPath = Paths.vocal(PlayState.songData.id, chartData.vocalsSuffix, '');
